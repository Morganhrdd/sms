...
for student in StudentYearlyInformation.objects.filter(ClassMaster__Standard = var1, ClassMaster__Division = var2, ClassMaster__AcademicYear__Year=yr):
    ...
    try:
        ...
        continue
    if student.ClassMaster.Standard != var1 and student.ClassMaster.Division != var2:
        continue
    '''
    ...
    for mark in marks:
        ...
        marks_dict[mark.TestMapping.SubjectMaster.Name][mark.TestMapping.TestType] = mark.MarksObtained
        #story.append(Paragraph(mark.TestMapping.SubjectMaster.Name+' --- '+mark.TestMapping.TestType+' === '+str(mark.MarksObtained), style))

    marks_data = []
    marks_data.append(['', 'Formative Evaluation', 'Summative Evaluation', 'Total', 'Grade'])
    grand_total = ['Total', 0, 0, 0, '']
    for subject in ['Marathi', 'Hindi/Sanskrit', 'English', 'Science', 'Mathematics', 'Social Science']:
        tmp = []
        total = 0
        x = 'FE1'
        if subject == 'Hindi/Sanskrit':
            subject = 'Sanskrit'
            print subject
        ...
        if x in marks_dict[subject]:
            try:
                tmp.append(marks_dict[subject][x])
                total += marks_dict[subject][x]
                grand_total[1] += marks_dict[subject][x]
            except:
        ...
        if x in marks_dict[subject]:
            try:
                tmp.append(marks_dict[subject][x])
                total += marks_dict[subject][x]
                grand_total[2] += marks_dict[subject][x]
            except:
                tmp.append('Data not available')
        else:
            tmp.append('')
        tmp.append(total)
        tmp.append(get_grade(total))
        marks_data.append(tmp)
    grand_total[3] = grand_total[1] + grand_total[2]
    marks_data.append(grand_total)
    percent = round(grand_total[3] / 6.0, 2)
    marks_data.append(['%', '', '', percent, get_grade(percent)])
    #marks_data.append(['', '', '', '', '', ])
    for subject in ['Drawing', 'Physical Education', 'Work Experience']:
        tmp = []
        total = 0
        tmp.append(subject)
        try:
            tmp.append(marks_dict[subject]['FE1'])
            total += marks_dict[subject]['FE1']
        except:
            tmp.append('Data not available')
        tmp.append('N/A')
        tmp.append(total)
